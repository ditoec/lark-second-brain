...
- **`/obsidian-lark-sync` + Lark Wiki adapter.** New command and `adapters/lark/` that sync the local markdown vault to Lark Wiki (飞书知识库). Local `.md` files remain the primary source of truth; `scripts/sync.ps1` handles bidirectional push/pull/sync and tracks state in `.lark-sync.json`. The adapter is an identity-copy build (no tool-name rewriting needed - it targets Claude Code) that bundles `sync.ps1` and emits `LARK.md` with the sync workflow reference. Space ID is configurable via `$env:LARK_WIKI_SPACE_ID` or a `lark_space_id:` field in `_CLAUDE.md`. Folder structure is mirrored as Lark Wiki parent nodes; conflict detection flags files changed on both sides since the last sync.

- **Four new Lark sync commands extending beyond Wiki to Base, Contacts, Calendar, and Tasks.** The vault now has a full Lark integration layer: tasks, assignments, schedules, meeting notes, and key persons sync bidirectionally with the Lark suite.

  - **`/obsidian-lark-base-sync`** - Sync structured vault notes to Lark Base (bitable) as database records. Creates four tables on `init` (Tasks, Projects, Persons, Decisions) with typed field schemas. Pushes `type: task`, `type: project`, `type: person`, and `type: decision` notes respectively. Pull syncs only frontmatter fields back to vault (vault body is authoritative). State in `.lark-base-sync.json`. Script: `scripts/lark-base-sync.ps1`.

  - **`/obsidian-lark-contacts-sync`** - Sync vault `type: person` notes to/from Lark Contacts. Deduplicates by email search before creating. Writes `lark_user_id:` back into person note frontmatter after first link so future syncs are exact. `import` sub-command pulls the full Lark directory and creates AI-first person stubs with empty body sections. External contacts (not in the Lark org directory) are flagged with a tip to use `/obsidian-lark-base-sync push persons` instead. State in `.lark-contacts-sync.json`. Script: `scripts/lark-contacts-sync.ps1`.

  - **`/obsidian-lark-calendar-sync`** - Sync meeting notes and task due dates to/from Lark Calendar. `push meetings` pushes `type: meeting` notes as calendar events; `push schedule` turns task `due:` dates into one-hour Lark events. `init` pulls the next 30 days of events as vault meeting stubs. Pulled stubs get empty Notes / Decisions / Action items sections (never fabricated). Attendees are resolved via person note `lark_user_id:` fields. Writes `lark_event_id:` back into note frontmatter after push. State in `.lark-calendar-sync.json`. Script: `scripts/lark-calendar-sync.ps1`.

  - **`/obsidian-lark-tasks-sync`** - Sync vault task notes to/from Lark Tasks with full assignee resolution and subtask support. Sources both `Tasks/` folder notes and kanban board items. Assignees are resolved to Lark user IDs via person note `lark_user_id:` fields (skips with a warning when absent rather than guessing). Subtask checklist items under `## Subtasks` are pushed as Lark subtasks. Status is mapped bidirectionally (todo/in-progress/blocked/done/cancelled <-> to_do/in_progress/completed/abandoned). Bidirectional `sync` detects conflicts when both sides changed since last sync and defers to user. Writes `lark_task_guid:` back into task frontmatter after first push. State in `.lark-tasks-sync.json`. Script: `scripts/lark-tasks-sync.ps1`.

- **`/obsidian-ingest` extended to accept local folders and Lark Wiki spaces.** Previously the command accepted only a single source (URL, file path, or pasted text). It now handles three source modes:
  - **Single source** (unchanged): URL, file path, or pasted text - existing pipeline.
  - **Local folder**: pass a directory path. The command scans recursively for supported file types (`.md`, `.pdf`, `.png`, `.jpg`, `.mp3`, `.m4a`, and others), deduplicates by `content_hash:` frontmatter against `raw/` so already-ingested files are skipped, reports an inventory before processing, then runs batched ingestion (default 10 files per batch) with parallel subagents per batch. After all batches a cross-batch synthesis pass finds entities and concepts spanning multiple files - patterns no single file could surface.
  - **Lark Wiki space**: pass a Lark Wiki URL (`https://*.larksuite.com/wiki/...`) or bare space ID. Fetches the full node tree via `lark wiki +node-list`, fetches each document as markdown via `lark docs +fetch --doc-format markdown`, mirrors the hierarchy to `raw/lark-wiki/<space-id>/`, and processes in batches grouped by parent node (sibling wiki pages share a batch to exploit topical locality). Cross-batch synthesis pass runs after all batches using the wiki's folder structure as a topical hint.
  - Optional flags added: `--dry-run` (preview inventory without writing), `--filter <glob>` (restrict folder scan), `--shallow` (top-level only, no recursion), `--batch-size <n>` (files per batch, default 10).
  - `triggers_en` extended with folder/wiki/bulk-ingest phrases.
