...
**Sync local vault notes to/from Lark Wiki.**

Local `.md` files are the primary source of truth. Lark Wiki is the sync target.
State (node tokens + content hashes) is tracked in `.lark-sync.json` at the vault root.

Sub-commands:

| Sub-command | What it does |
|---|---|
| `status` (default) | Show new, modified, moved, and in-sync files |
| `push` | Push all locally changed files to Lark |
| `push <rel-path>` | Push one file |
| `pull` | Pull all tracked Lark nodes to local |
| `pull <rel-path>` | Pull one file |
| `sync` | Bidirectional: push local changes, pull Lark changes, flag conflicts |
| `init` | First-time push of every local `.md` file to the configured Wiki space |

Steps:
1. Read `_CLAUDE.md` for vault root and optional `lark_space_id:` field
2. If `lark_space_id` is present, set `$env:LARK_WIKI_SPACE_ID` before running
3. Run `pwsh -File scripts/sync.ps1 <sub-command> [file] -VaultRoot <vault-root>` via the Bash tool
4. Report result: files pushed/pulled, conflicts, or status summary

First-time setup: if `.lark-sync.json` does not exist, ask for the Lark space ID then run `init`.
Conflict resolution: when `sync` reports a conflict, ask which version to keep, then use `push` or `pull` to resolve.

---

### `/obsidian-lark-base-sync`

**Sync vault structured data (tasks, projects, persons, decisions) to/from Lark Base (bitable) as database records.**

State tracked in `.lark-base-sync.json`. Tables are created automatically on first `init`.

| Sub-command | What it does |
|---|---|
| `status` (default) | Show pending pushes, modified records per table |
| `init` | Create the four Lark Base tables and push all current vault notes |
| `push [type]` | Push all changed notes of that type (tasks/projects/persons/decisions), or all four if omitted |
| `push <rel-path>` | Push a single note by vault path |
| `pull [type]` | Pull Lark Base records back to vault frontmatter |
| `sync` | Bidirectional: push local changes, pull Lark changes |

Steps:
1. Read `_CLAUDE.md` for vault root and optional `lark_base_app_token:` field
2. If present, set `$env:LARK_BASE_APP_TOKEN` before running
3. Run `pwsh -File scripts/lark-base-sync.ps1 <sub-command> [type] -VaultRoot <vault-root>` via the Bash tool
4. Report result: records created/updated, tables initialized, or status summary

Note type to table mapping: `type: task` -> Tasks, `type: project` -> Projects, `type: person` -> Persons, `type: decision` -> Decisions.

---

### `/obsidian-lark-contacts-sync`

**Sync vault person notes to/from Lark Contacts - push keypersons, pull contact updates.**

State tracked in `.lark-contacts-sync.json`. Links notes to Lark users via `lark_user_id:` frontmatter.

| Sub-command | What it does |
|---|---|
| `status` (default) | Show which person notes are new, modified, or in sync |
| `push` | Push all changed person notes to Lark Contacts |
| `push <rel-path>` | Push a single person note |
| `pull` | Pull all tracked Lark users back to vault |
| `pull <user-id>` | Pull one Lark contact by user ID |
| `sync` | Bidirectional sync |
| `import` | One-way: pull all Lark Contacts, create missing vault person stubs |

Steps:
1. Read `_CLAUDE.md` for vault root
2. Run `pwsh -File scripts/lark-contacts-sync.ps1 <sub-command> [path|id] -VaultRoot <vault-root>` via the Bash tool
3. Report result: contacts synced, stubs created, or status

Note: Lark Contacts API updates org members. For external contacts, use `/obsidian-lark-base-sync push persons` instead.
Writes `lark_user_id:` back into person note frontmatter after first link. Imported stubs include empty `## Context`, `## Interactions`, `## Notes` sections - never fabricated.

---

### `/obsidian-lark-calendar-sync`

**Sync vault meeting notes and task due dates to/from Lark Calendar.**

State tracked in `.lark-calendar-sync.json`. Writes `lark_event_id:` back into note frontmatter after push.

| Sub-command | What it does |
|---|---|
| `status` (default) | Show unsynced meetings and tasks with unscheduled due dates |
| `init` | Pull all upcoming Lark events (next 30 days) as vault meeting stubs |
| `push meetings` | Push `type: meeting` notes to Lark Calendar as events |
| `push schedule` | Push task `due:` dates as one-hour Lark Calendar events |
| `push <rel-path>` | Push a single meeting note |
| `pull` | Pull all tracked events back to vault |
| `pull <event-id>` | Pull one event by its Lark event ID |
| `sync` | Bidirectional: push vault changes, pull Lark changes |

Steps:
1. Read `_CLAUDE.md` for vault root and optional `lark_calendar_id:` field
2. If present, set `$env:LARK_CALENDAR_ID`; else primary calendar is used
3. Run `pwsh -File scripts/lark-calendar-sync.ps1 <sub-command> [target] -VaultRoot <vault-root>` via the Bash tool
4. Report result: events created, stubs written, or status

Pulled event stubs are created at `wiki/meetings/YYYY-MM-DD - <Title>.md` with empty Notes / Decisions / Action items sections. Attendees are cross-linked to `[[Person]]` notes. Never fabricate meeting content.

---

### `/obsidian-lark-tasks-sync`

**Sync vault task notes and kanban board items to/from Lark Tasks with assignee resolution.**

State tracked in `.lark-tasks-sync.json`. Writes `lark_task_guid:` back into task frontmatter after first push.
Sources both `Tasks/` folder (`type: task` notes) and kanban boards (`kanban-plugin: board` files).

| Sub-command | What it does |
|---|---|
| `status` (default) | Show unsynced tasks and modified tasks |
| `init` | First-time push of every vault task to Lark Tasks |
| `push` | Push all changed tasks |
| `push <rel-path>` | Push a single task note |
| `pull` | Pull all tracked Lark tasks back to vault |
| `pull <task-guid>` | Pull one Lark task by GUID |
| `sync` | Bidirectional: push local, pull Lark, flag conflicts |

Steps:
1. Read `_CLAUDE.md` for vault root
2. Run `pwsh -File scripts/lark-tasks-sync.ps1 <sub-command> [path|guid] -VaultRoot <vault-root>` via the Bash tool
3. Report result: tasks created/updated/pulled, conflicts flagged, or status

Assignee resolution: looks up each `assignee:` value in `People/<name>.md`; uses that note's `lark_user_id:` field. Skips assignee field (with a warning) rather than guessing when `lark_user_id:` is absent.
Subtasks: checklist items under `## Subtasks` in a task note are pushed as Lark subtasks.
Status mapping: `todo` -> to_do, `in-progress` -> in_progress, `blocked` -> in_progress + [BLOCKED] tag, `done` -> completed, `cancelled` -> abandoned.
